...
def test_commit_not_DB(init_database):
    with pytest.raises(Exception) as ex:
        crud.get_by_sha(init_database, 2)
...
def test_get_by_sha(init_database):
    # Test calling existing commit returns expected data
    response = crud.get_by_sha(init_database, "daffasdfsjfoie3039j33j882ji2jhsdaf")
    ...
    with pytest.raises(Exception) as ex:
        response = crud.get_by_id(init_database, "ladsjkflasjkdaldsfjl234lfdahfa")
        assert ex.value.message == "Item not found"


def test_create_commit(init_database):
    ...
    commit = init_database.get(Commit, response)
    ...
    with pytest.raises(Exception) as ex:
        response = crud.create_commit(
            init_database, commit_data, project_auth_token="WrongToken"
        )
...
# Test delete_commit
def test_delete_commit(init_database):
    # Test exception thrown for non-existant commit
    with pytest.raises(Exception) as ex:
        response = crud.delete_commit(
            init_database, "akdfjlaqeworuoifa123", project_auth_token="Catalyst"
        )
        assert ex.value.message == "Item not found"
    ...
    assert init_database.get(Commit, "daffasdfsjfoie3039j33j882ji2jhsdaf") is None


# Test update_commit
def test_update_commit(init_database):
    ...
    commit = init_database.get(Commit, response.sha)
    assert commit.date == datetime.datetime(2021, 8, 11, 9, 43, 8, 41351)
    assert commit.message == "feat(test): a new commit message"
    assert commit.author == "Spider-girl"
    assert commit.workItemId == 1
